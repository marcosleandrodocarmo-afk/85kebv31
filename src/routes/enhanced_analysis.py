...
def analyze_ultra_enhanced():
    ...
    try:
        ...
        forensic_metrics = _calculate_comprehensive_forensic_metrics(final_analysis)
        final_analysis['metricas_forenses_ultra_detalhadas'] = forensic_metrics

        # Gera relatório arqueológico final
        archaeological_report = _generate_comprehensive_report(final_analysis)
...
@enhanced_analysis_bp.route('/analyze_ultra_enhanced_duplicate', methods=['POST'])
def analyze_ultra_enhanced_duplicate():
...
@enhanced_analysis_bp.route('/api/analyze', methods=['POST'])
def analyze_enhanced():
    """Rota para análise enhanced"""
    try:
        ...
        # Aqui você chamaria a lógica de análise real
        # Por enquanto, vamos simular um ID de análise e retornar o session_id
        analysis_id = f"analysis_{int(time.time() * 1000)}"

        # Simula o início da análise e salva o progresso
        update_analysis_progress(session_id, 0, "Análise iniciada")
        salvar_etapa("inicio_analise_enhanced", {
            "session_id": session_id,
            "input_data": data,
            "timestamp": datetime.now().isoformat()
        }, categoria="logs")

        # Simula a execução de agentes (substituir pela lógica real)
        # Ex: resultado_agente1 = agente1.execute(data, session_id)
        # Ex: resultado_agente2 = agente2.execute(data, session_id)

        # Atualiza progresso para 100% após simulação
        update_analysis_progress(session_id, 100, "Análise concluída")
        salvar_etapa("fim_analise_enhanced", {
            "session_id": session_id,
        ...
        logger.error(f"❌ Erro na análise enhanced: {e}")
        return jsonify({
            "success": False,
            "error": str(e),
